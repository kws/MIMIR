...
/**
 * High-priority timer manager for RTP packet timing.
 * Uses a dedicated thread pool with real-time scheduling to ensure precise timing
 * independent of Vert.x event loop performance.
 */
public class RTPTimerManager {
    ...
    public void startPeriodicTask(long intervalMs, Runnable task) {
        if (isRunning.compareAndSet(false, true)) {
            ...
            rtpTask = rtpScheduler.scheduleAtFixedRate(() -> {
                ...
                if (jitter > 5) {
                    LOG.warn("RTP timer jitter detected: {}ms (expected: {}ms, actual: {}ms)", 
                            jitter, intervalMs, actualInterval);
                }
    ...
    public void stop() {
        if (isRunning.compareAndSet(true, false)) {
            ...
            if (packets > 0) {
                ...
                LOG.info("RTP timer stopped. Packets sent: {}, Average jitter: {}ms", 
                        packets, String.format("%.2f", avgJitter));
            }
    ...
    public static class TimingStats {
        ...
        public String toString() {
            return String.format("TimingStats{packets=%d, avgJitter=%.2fms}", 
                    totalPackets, averageJitterMs);
        }
    }
} 